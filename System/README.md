...
```
Read ~/MBP-Mods/KindasMD/System/README.md -- it tells you everything.
Check System/handoffs/ for the latest session note.
The active plan is at System/plans/[exact-filename].md.
...
2. Read `System/AGENTS.md` for architecture and technical context
3. Read the latest handoff in `System/handoffs/` for current state
4. Read the active plan in `System/plans/` for your session's tasks
5. Build: `bash System/build.sh` (then Dock-launch `KindasMD.app` to verify)
6. Do the work, following the plan section for your session exactly
7. After work: write handoff, commit, push, give user the cold-start
...
- The repo must always reflect the current SOT state.
...
```
KindasMD/
├── KindasMD.app           ← Dock launcher (the only clickable thing)
...
- Launcher: `KindasMD.app` (Dock) → `System/scripts/kindasmd` → `open -na System/app/KindasMDEditor.app`
